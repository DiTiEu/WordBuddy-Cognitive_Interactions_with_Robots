...
class CNNConfig:
    ...
    input_size: int = 64
    grayscale: bool = True  # <--- QUESTA E' LA RIGA CHE MANCAVA
    min_confidence: float = 0.25
    return_unknown_as_empty: bool = True

    # Occupancy safety net
...
class SlotClassifierCNN:
    ...
    def load(self) -> bool:
        ...
        try:
            ...
            if self.cfg.verbose_load:
                print("[CNN] Loaded:", self.cfg.model_dir)
            return True
    ...
    def _preprocess(self, slot_bgr: np.ndarray) -> np.ndarray:
        ...
        # Usa il parametro di configurazione
    ...
    def _is_empty(self, pre: np.ndarray) -> bool:
        ...
        y0 = max(0, cy - half)
        y1 = min(S, cy + half)
        center = pre[y0:y1, x0:x1, :]
        m = float(np.mean(center) * 255.0)
    ...
    def predict(self, slot_bgr: np.ndarray) -> Tuple[str, float, Optional[np.ndarray]]:
        if self.model is None:
            ok = self.load()
            if not ok:
                # Se il modello non c'è ancora, restituisce '?'
                return "?", 0.0, None
        ...
        if self._is_empty(pre):
            return " ", 100.0, (pre[:, :, 0] * 255).astype(np.uint8)

        x = np.expand_dims(pre, axis=0)
        probs = self.model.predict(x, verbose=0)[0]
        j = int(np.argmax(probs))
        conf = float(probs[j])
        pred = CLASSES[j]  # '_' or 'A'..'Z'
        ...
        return out, conf * 100.0, (pre[:, :, 0] * 255).astype(np.uint8)